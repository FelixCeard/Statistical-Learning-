...
\begin{align*}
    ...
    &= 1 + \pi \left(6a - 4a^2 - 2 \right) + \pi^2 (4 a^2 - 4 a + 1) - 2a + a^2\;.
\end{align*}
...
\begin{equation*}
    \pi_{naive} = \frac{\sum_{i=1}^{n} X_i}{n}\;.
\end{equation*}
...
We have
\begin{align*}
    \mathbb{E}[\pi_{naive}] &= \mathbb{E}\left[ \frac{\sum_{i=1}^{n} X_i}{n} \right] \\
    ...
    &= \pi \cdot (1-\ell)\;.
\end{align*}
Therefore, the naive estimator of $\pi$ is only unbiased whenever $\ell = 0$. Otherwise, the estimator is biased towards 0 the more the persons of the sensitive group lie ($\lim_{\ell \rightarrow 1} \mathbb{E}[\pi_{naive}] = 0$).
